...
//Scene constants
const int numberOfSceneObjects = 2;
...
const vec3 lightSource = vec3(30.0,40.0,-40.4);
...
const Scene globalScene = Scene(Object[numberOfSceneObjects](
	Object(vec3(0.0,1.5,0.0),1.5,1,vec3(1.0,1.0,1.0),0,0), //plane -> the "room" of the scene
	Object(vec3(0.0,.35,0.0),0.7,0,vec3(0.0,.7,0.0),0,1) //sphere -> one of the objects in the scene
	//Object(vec3(1.0,0.6,0.0),0.7,2,vec3(0.9,0.0,0.0),0,2), //cone -> one of the objects in the scene
	//Object(vec3(2.0,0.5,0.0),0.7,3,vec3(0.3,0.3,1.0),0,1) //solid cube -> one of the objects in the scene
));
...
March rayMarchScene(vec3 from, vec3 direction) {
	...
	for (int steps = 0; steps < MAX_MARCHING_STEPS; steps++){
		...
		if(sceneCollision.distance > MAX_DIST || sceneCollision.distance < EPSILON*0.0001) break;
	}
...
vec3 getReflection(vec3 surfacePoint, vec3 rayDirection, vec3 normal){
	...
	vec3 color = rayMarchScene(surfacePoint+normal*EPSILON*4,reflectionDirection).color;
	return color;
}
...
void main(){
	...
	if(marchToScene.distance >= MAX_DIST){
		...
		vec3 finalColor = marchToScene.color*(diffuse+1.0)*clamp(shadow*1.5,0.3,1.0)*(ambientOcclusion-0.1)+(specular*0.4);
